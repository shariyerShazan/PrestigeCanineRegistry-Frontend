  ...
  const stats = [
    {
      ...
      value: apiData?.totalPublished || "00",
      bg: "bg-[#D4F5E9]",
      border: "border-[#24B57B]",
      text: "text-[#24B57B]",
      icon: <LuDog size={28} />,
    },
    {
      label: "Gold Verified",
      value: apiData?.goldVerified || "00",
      bg: "bg-[#FEF3C7]",
    ...
    {
      label: "Blue Verified",
      value: apiData?.blueVerified || "00",
      bg: "bg-[#DBEAFE]",
      border: "border-[#2B4C8A]",
      text: "text-[#2B4C8A]",
      icon: <Award size={28} />,
    },
    {
      label: "Pending",
      value: apiData?.pending || "00",
      bg: "bg-[#F3F4F6]",
    ...
    {
      label: "Rejected",
      value: apiData?.rejected || "00",
      bg: "bg-[#FEE2E2]",
      border: "border-[#D45637]",
      text: "text-[#D45637]",
      icon: <Ban size={28} />,
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {stats.map(({ label, value, bg, border, text, icon }) => (
            <div
              key={label}
              className={`rounded-2xl border-1 p-5 flex justify-between ${bg} ${border}`}
            >
              ...
              <div className="flex flex-col justify-end items-end mb-2">
                <div className={`text-3xl font-bold ${text}`}>
                  {statsLoading ? "..." : String(value).padStart(2, "0")}
                </div>
        ...
        <div className="flex justify-end gap-3 mb-6">
          <Button
            ...
            variant="outline"
            className="border-[#2B4C8A] text-[#2B4C8A] cursor-pointer"
          >
            Transfer Ownership
          </Button>
          <Button
            onClick={() => navigate("/owner/dashboard/litter-registration")}
            className="bg-[#2B4C8A] text-white cursor-pointer"
          >
            Register Litter
          </Button>
          <Button
            onClick={() => navigate("/owner/dashboard/dog-registration")}
            className="bg-[#D4AF37] text-black font-semibold cursor-pointer hover:text-white"
          >
            Register New Dog ⊕
          </Button>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {tabs.map((t) => (
            <TabsContent key={t.value} value={t.value}>
              <AllPublishedDogs activeFilter={t.value} />
            </TabsContent>
          ))}
        </Tabs>
