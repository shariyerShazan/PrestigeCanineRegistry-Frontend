                  ...
                  <div className="w-full md:w-2/3 flex flex-col gap-6">
                    <div>
                      <div className="flex items-center gap-3 mb-4 flex-wrap">
                        ...
                        {canine?.status && (
                           <Badge className={canine.status === "ACTIVE" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}>
                              {canine.status}
                           </Badge>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <DataBox label="PCR ID" value={canine?.pcrId || "N/A"} valueColor="text-blue-600 font-mono" />
                        <DataBox label="Breed" value={canine?.breedRelation?.name || "N/A"} valueColor="text-slate-800" />
                        <DataBox label="Generation" value={canine?.generation || "PUREBRED"} valueColor="text-slate-800" />
                        <DataBox label="Gender" value={canine?.gender || "N/A"} valueColor="text-amber-600" />
                        <DataBox label="Microchip" value={canine?.microchipId || "N/A"} valueColor="text-blue-600 font-mono" />
                        <DataBox label="DOB" value={formatDate(canine?.dateOfBirth)} valueColor="text-slate-800" />
                        <DataBox label="Weight" value={canine?.weight ? `${canine.weight} KG` : "N/A"} valueColor="text-slate-800" />
                        <DataBox label="Color" value={canine?.color || "N/A"} valueColor="text-slate-800" />
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white bg-slate-200">
                             {canine.owner.profileImage && (
                               <img src={canine.owner.profileImage} className="w-full h-full object-cover" />
                             )}
                          </div>
                          <div>
                            <h3 className="text-base font-bold text-slate-800">{canine.owner.fullName}</h3>
                            <div className="text-xs text-slate-500 font-medium mt-1 flex flex-col sm:flex-row sm:gap-4 gap-1">
                               <p className="flex items-center gap-1"><MapPinIcon size={12} /> {canine.city}, {canine.country}</p>
                               <p>PCR ID: <span className="font-mono text-blue-600">{canine.owner.pcrId}</span></p>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="bg-[#1A1A1A] text-white rounded-xl p-6 relative overflow-hidden shadow-xl min-h-75">
                  <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-bold text-lg tracking-wide">DNA Report</h4>
                    </div>
                  ...
                  <div className="space-y-6 mt-4">
                    <div>
                      ...
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-bold text-base">{canine?.breedRelation?.name || "N/A"}</span>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                >
                  <div>
                    ...
                    <p className="font-bold text-slate-800 text-lg">{canine?.litter?.father?.name || "Unknown Lineage"}</p>
                    <p className="text-sm font-mono text-blue-600 mt-1">{canine?.litter?.father?.pcrId || "—"}</p>
                  </div>
                ...
                >
                  <div>
                    ...
                    <p className="font-bold text-slate-800 text-lg">{canine?.litter?.mother?.name || "Unknown Lineage"}</p>
                    <p className="text-sm font-mono text-blue-600 mt-1">{canine?.litter?.mother?.pcrId || "—"}</p>
                  </div>
                  <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm group-hover:bg-amber-100 group-hover:text-amber-600 transition-colors">
                     <FiArrowRight size={18} />
                  </div>
